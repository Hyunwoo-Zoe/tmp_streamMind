...
torch.set_num_threads(int(os.getenv("STREAMMIND_TORCH_THREADS", "16")))
torch.set_num_interop_threads(int(os.getenv("STREAMMIND_TORCH_INTEROP", "1")))

class NVMLSampler:
    def __init__(self, device_index=0, interval_ms=20):
    ...
    def start(self):
        ...
        def _loop():
            while not self.stop_ev.is_set():
                ...
                self.utils.append(int(u.gpu))              # %
                self.mem_used.append(int(m.used))         # bytes
                time.sleep(self.interval)

        self.th = Thread(target=_loop, daemon=True)
        self.th.start()
    ...
    def stats(self):
        # util: avg/p95/max, mem_used: max(MB)
        if not self.utils:
            return None, None, None, None
        u = np.array(self.utils, dtype=np.float32)
        mu = np.array(self.mem_used, dtype=np.float64)
        util_avg = float(np.mean(u))
        util_p95 = float(np.percentile(u, 95))
        util_max = float(np.max(u))
        mem_peak_mb = float(np.max(mu) / (1024.0**2))
        return util_avg, util_p95, util_max, mem_peak_mb
...
def run_one_setting(model, processor, tokenizer, version, vr, sampling_fps, steps, warmup, realtime, instruct, gate_mode, rows, repeat=1):
    ...
    for j, frame_id in enumerate(idxs):
        ...
        with torch.inference_mode():
            ...
            sampler = NVMLSampler(device_index=0, interval_ms=20)
            sampler.start()
            ...
            sampler.stop()
            util_avg, util_p95, util_max, nvml_mem_peak_mb = sampler.stats()
            ...
            if torch.cuda.is_available():
                ...
                delta_peak_alloc_mb = float((torch.cuda.max_memory_allocated() - base_alloc) / (1024.0**2))
            else:
                peak_alloc_mb, peak_reserved_mb, delta_peak_alloc_mb = None, None, None

        # 이후 row에는 last_metrics 사용
        cls_pred = last_cls_pred
        ...
        rows.append({
            ...
            "nvml_mem_peak_mb": nvml_mem_peak_mb,
            "gpu_util_avg": util_avg,
            "gpu_util_p95": util_p95,
            "gpu_util_max": util_max,

            "delta_peak_alloc_mb": delta_peak_alloc_mb,
            "repeat": int(repeat),
...
def main():
    ...
    ap.add_argument("--repeat", type=int, default=1,
                help="Repeat stream_generate N times per frame to stabilize NVML util (e.g., 200)")
    ...
    for mode in gate_modes:
        ...
        for fps in fps_list:
            run_one_setting(model, processor, tokenizer, version, vr,
                ...
                args.instruct, mode, rows, repeat=args.repeat)


    # 1. Raw Data 저장 (필드 추가)
    raw_path = os.path.join(args.outdir, "raw.csv")
    fields = [
        "gate_mode","sampling_fps","frame_id","cls_pred","wall_ms",
        "clip_ms","offload_ms","xfer_gate_ms","gate_comp_ms","mamba_ms","trans_ms",
        "peak_alloc_mb","peak_reserved_mb","delta_peak_alloc_mb","nvml_mem_peak_mb",
        "gpu_util_avg","gpu_util_p95","gpu_util_max",
        "repeat",
        ]
