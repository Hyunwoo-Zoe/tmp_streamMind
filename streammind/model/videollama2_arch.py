...
class Videollama2MetaForCausalLM(ABC):
    ...
    def encode_images_or_videos_score_cls_inference_allframe(
        ...
        # [B, T, N, H]
        frames_features = einops.rearrange(frames_features, "(b t) n h -> b t n h", b=batch_size)
        frames_features_new = frames_features  # [B, T_new, N, H]
        if past_frames_features is None:
            self._gate_step_state = None

        T_new = int(frames_features_new.shape[1])
        ...
        if gate_mode == "cpu":
            ...
            frames_features = frames_features_new
        ...
        self._dbg_total_t_before_max_t = int(cur_total_t)
        self._dbg_max_t = int(MAX_T)
        self._dbg_t_after_max_t = int(frames_features.shape[1])
        self._dbg_frames_shape_after_max_t = list(map(int, frames_features_shape or []))

        # ✅ MAX_T 적용 이후 "현재 남아있는 총 길이" 기준으로 ΔT 다시 산출
        T_total = int(frames_features.shape[1])
        T_new_eff = min(T_new, T_total)  # 안전장치
        frames_features_new_eff = frames_features[:, -T_new_eff:].contiguous()
        ...
        frames_features_shape_for_gate = [int(frames_for_gate.shape[1])]
        if os.getenv("STREAMMIND_DBG_SHAPE", "0") == "1":
            print(
                f"[DBG_SHAPE] totalT={int(frames_features.shape[1])} "
                f"gateT={int(frames_for_gate.shape[1])} "
                f"shape_len={len(frames_features_shape_for_gate)} "
                f"shape={frames_features_shape_for_gate}"
            )
        frames_features_shape_for_gate_new = [int(frames_features_new_eff.shape[1])]

        # -----------------------------
        # 6) projector 선택 + CPU projector build
        # -----------------------------
        if gate_mode == "cpu":
            gate_dev = torch.device("cpu")

            need_build = (not hasattr(self, "_mm_projector_cpu")) or (self._mm_projector_cpu is None)
            cached_dtype = getattr(self, "_mm_projector_cpu_dtype", None)

            if need_build or (cached_dtype != cpu_dtype):
                print(f"[INFO] Building CPU Projector (IPEX, dtype={cpu_dtype}, has_ipex={_HAS_IPEX})...")

                # ✅ m 정의는 반드시 여기서
                m = copy.deepcopy(self.get_model().mm_projector).to("cpu", dtype=cpu_dtype)
                m.eval()
            ...
            gate_dev = next(mm_proj.parameters()).device
        ...
        # (B) incremental 경로용: 이번 chunk만
        frames_new_for_gate = frames_features_new_eff
        frames_new_for_gate_dev = frames_new_for_gate
        ...
        print(f"[XFER_GATE] full={xfer_gate_ms:.2f} ms  new={xfer_gate_new_ms:.2f} ms  "
            f"MB_full={_mb(frames_for_gate):.2f}  MB_new={_mb(frames_new_for_gate):.2f}")
        ...
        def _get_step_method(model):
            # 1. 모델 자체가 step을 가지고 있는 경우
            if hasattr(model, "step"):
                return model.step
            # 2. IPEX나 DDP 등으로 감싸져서 .module 안에 있는 경우
            if hasattr(model, "module") and hasattr(model.module, "step"):
                return model.module.step
            # 3. Torch Compile 등으로 감싸진 경우 (_orig_mod)
            if hasattr(model, "_orig_mod") and hasattr(model.orig_mod, "step"):
                return model._orig_mod.step
            return None

        INCR_GATE = (os.getenv("STREAMMIND_GATE_INCREMENTAL", "0") == "1")
        step_fn = _get_step_method(mm_proj) # 여기서 추출한 함수를 써야 함!

        if INCR_GATE and os.getenv("STREAMMIND_DBG_INCR", "0") == "1":
            print(f"[DBG_INCR] Attempting Incremental...")
            print(f"   - mm_proj type: {type(mm_proj)}")
            print(f"   - Final Decision: Use STEP? {step_fn is not None}")
            
        def _run_projector_step(frames_feat_new_dev: torch.Tensor):
            """
            frames_feat_new_dev: [B, T_new, N, H] on gate_dev
            returns: (X_last, cls_last)
            """
            state = getattr(self, "_gate_step_state", None)
            X_last, cls_last = None, None
            
            # [최적화] T_new=1 (대부분의 경우) 일 때 Loop 오버헤드 제거
            if frames_feat_new_dev.shape[1] == 1:
                 # [중요] mm_proj.step 대신 step_fn 사용
                 out_i = step_fn(frames_feat_new_dev, state=state, frames_features_shape=frames_features_shape_for_gate_new)
                 
                 # 리턴값 파싱
                 if isinstance(out_i, (tuple, list)):
                     if len(out_i) == 3: X_last, cls_last, new_state = out_i
                     elif len(out_i) == 2: X_last, cls_last = out_i; new_state = state
                     else: X_last, cls_last = out_i[0], None; new_state = state
                 else:
                     X_last, cls_last = out_i, None; new_state = state
                 state = new_state
            
            else:
                # Initial Chunk (T > 1) 루프
                for i in range(frames_feat_new_dev.shape[1]):
                    x_i = frames_feat_new_dev[:, i:i+1]
                    
                    # [중요] mm_proj.step 대신 step_fn 사용
                    out_i = step_fn(x_i, state=state, frames_features_shape=frames_features_shape_for_gate_new)

                    if isinstance(out_i, (tuple, list)):
                        if len(out_i) == 3: X_i, cls_i, new_state = out_i
                        elif len(out_i) == 2: X_i, cls_i = out_i; new_state = state
                        else: X_i, cls_i = out_i[0], out_i[1] if len(out_i) > 1 else None; new_state = state
                    else:
                        X_i, cls_i = out_i, None; new_state = state
                    
                    state = new_state
                    X_last, cls_last = X_i, cls_i

            self._gate_step_state = state
            return X_last, cls_last
        ...
        try:
            # [수정] 조건문을 'step_fn 존재 여부'로 변경
            if INCR_GATE and (step_fn is not None):
                if gate_dev.type == "cpu":
                    with torch.inference_mode(), torch.amp.autocast(device_type="cpu", dtype=cpu_dtype, enabled=True):
                        X_last, cls_last = _run_projector_step(frames_new_for_gate_dev)
                else:
                    X_last, cls_last = _run_projector_step(frames_new_for_gate_dev)
                ...
                # Fallback: INCR_GATE가 켜져있는데 step을 못 찾은 경우 경고
                if INCR_GATE:
                     print("[WARN] INCR_GATE=1 but step() method not found inside Wrapper! Falling back to O(T) mode.")
        ...
        # mm_proj가 래퍼면 last_mamba_ms가 갱신 안 됐을 수 있음 -> step_fn.__self__ 참조
        final_mamba, final_gate = 0.0, 0.0
        
        # 1. Wrapper에서 시도
        final_mamba = getattr(mm_proj, "last_mamba_ms", 0.0)
        final_gate = getattr(mm_proj, "last_gate_ms", 0.0)
        
        # 2. Wrapper가 값을 못 가져왔고 step_fn이 있다면, 원본 객체에서 가져오기
        if final_mamba == 0 and step_fn is not None and hasattr(step_fn, "__self__"):
             inner_model = step_fn.__self__
             final_mamba = getattr(inner_model, "last_mamba_ms", 0.0)
             final_gate = getattr(inner_model, "last_gate_ms", 0.0)

        # Fallback
        if final_mamba == 0 and final_gate == 0:
             final_trans = gate_compute_ms
             final_mamba = 0.0
        else:
             final_trans = final_gate

        print(f"[CLIP] {clip_ms:.2f} ms  [OFFLOAD] {offload_ms:.2f} ms  [GATE_COMP] {gate_compute_ms:.2f} ms "
              f"(Mamba: {final_mamba:.1f}, Trans: {final_trans:.1f})")
        ...
        # debug metrics 저장 (mistral.py가 가져갈 수 있게)
        ...
        # [중요] 세부 메트릭 추가
        self._dbg_mamba_ms = float(final_mamba)
        self._dbg_trans_ms = float(final_trans)
