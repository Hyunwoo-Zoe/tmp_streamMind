...
class PureMambaBlock(nn.Module):
    ...
    # [ssm.py] PureMambaBlock 클래스 내부에 추가

    def step(self, x, conv_state, ssm_state):
        """
        PureMambaBlock용 O(1) 증분 처리 (Recurrent Step)
        x: [B, 1, D]
        conv_state: [B, D_inner, D_conv]
        ssm_state: [B, D_inner, D_state]
        """
        # 1. Input Projection
        xz = self.in_proj(x.squeeze(1)) # [B, D_inner * 2]
        x, z = xz.chunk(2, dim=-1)

        # 2. Streaming Convolution (Time-shift)
        # 캐시를 한 칸 밀고(roll) 새 값을 끝에 넣음
        conv_state.copy_(torch.roll(conv_state, shifts=-1, dims=-1))
        conv_state[:, :, -1] = x
        
        # Conv1d 연산: sum(state * weight) + bias
        ...
        # 3. SSM Step (Selective Scan)
        x_dbl = self.x_proj(x_conv) # [B, dt_rank + d_state*2]
        dt, B, C = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1)
        
        dt = F.softplus(self.dt_proj(dt))
        A = -torch.exp(self.A_log.float()) # [D_inner, D_state]

        # Discretization & State Update (여기가 핵심 가속 포인트)
        # h_t = exp(dt * A) * h_{t-1} + (dt * B) * x_t
        deltaA = torch.exp(dt.unsqueeze(-1) * A) # [B, D_inner, D_state]
        deltaB = dt.unsqueeze(-1) * B.unsqueeze(1) # [B, D_inner, D_state]
        
        ssm_state.copy_(ssm_state * deltaA + deltaB * x_conv.unsqueeze(-1))
...
class VideoMamba(L.LightningModule):
    ...
    def __init__(self, config: SSMConfig, omit_in_proj: bool = False):
        ...
        # [중요] norm_fn은 if/else 바깥에 들여쓰기가 맞춰져 있어야 합니다!
